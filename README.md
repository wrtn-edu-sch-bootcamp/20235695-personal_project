...
종이 검수표를 하나씩 눈으로 확인하는 기존 방식 대신, 바코드 스캔과 자동 매칭으로 재고 확인 시간을 대폭 줄여줍니다.
...
## 현재 문제점
- 종이 검수표를 눈으로 보면서 물품을 하나씩 대조해야 함
...
### 1단계: 검수표 디지털화
- 종이 검수표를 카메라로 촬영하거나 파일(엑셀/CSV)로 업로드
...
- 실물 수량을 직접 입력
- 검수표 수량 vs 실물 수량을 자동 비교
  - ✅ 일치 → 확인 완료
  - ⚠️ 불일치 → 경고 및 차이 표시

## 주요 기능
- **검수표 업로드**: 사진 촬영(OCR) 또는 엑셀/CSV 파일 업로드
- **바코드 스캔**: 스마트폰 카메라로 상품 바코드 인식 (순서 상관없음)
- **수량 입력**: 바코드 스캔 후 실물 수량 직접 입력
- **자동 대조**: 검수표 수량과 실물 수량 자동 비교
